...
/// returned code for tokens, and stash a minimal user identity in the session. Same flow as
/// catalog-web's AuthController - see that file's doc comment for why `/login` exists as a
/// pre-redirect landing page.
struct AuthController: RouteCollection {
  ...
  func callback(req: Request) async throws -> Response {
    ...
    req.currentUser = SessionUser(
      sub: sub,
      name: (claims["name"] as? String) ?? (claims["email"] as? String) ?? "Admin",
