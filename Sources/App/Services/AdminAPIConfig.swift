...
/// Base URL for admin-api, the Go backend this app manages banner messages through. Calls happen
/// server-to-server (this app's pods to admin-api's ClusterIP Service), so this defaults to an
/// in-cluster DNS name - not a public ingress host - matching BackendConfig.swift's pattern in
/// catalog-web. Override via env var for local development against a port-forwarded or public
/// dev endpoint.
