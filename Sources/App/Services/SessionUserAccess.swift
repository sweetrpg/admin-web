...
extension Request {
  ...
  /// `ResilientRedisSessionDriver` gave this app before its own copy of it was removed in favor
  /// of this read-only client. This intentionally does NOT go through Vapor's
  /// `Session`/`SessionsMiddleware` machinery: touching `req.session` on every request would
  /// create and write back a brand-new session for every anonymous visitor, which is exactly the
