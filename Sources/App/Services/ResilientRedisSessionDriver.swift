...
/// request - so a Redis outage would 500 the whole app.
///
/// Reads fail open to "no session" (the request proceeds as logged-out, which then redirects to
/// login via AuthRequiredMiddleware); writes and deletes are logged and swallowed so the response
/// still succeeds, at the cost of the session not surviving until Redis recovers.
