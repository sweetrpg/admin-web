...
/// Same Auth0 Authorization Code flow as catalog-web's Auth0Config - see AuthController.swift.
///
/// admin-web needs its own Auth0 *application* (client id/secret/callback URL) registered within
/// the same tenant catalog-web uses, not a shared client id - each Vapor app owns its own
/// callback URL and Auth0 requires that to be registered per-application. That registration is a
/// one-time step in the Auth0 dashboard/Management API, not something this codebase can do for
/// itself; see this repo's README for the follow-up.
struct Auth0Config {
  let domain: String
  let clientID: String
